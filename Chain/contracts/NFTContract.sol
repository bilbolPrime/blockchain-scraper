...
contract NFTContract is Initializable, ERC721EnumerableUpgradeable, AccessControlUpgradeable {
    ...
    event NFTMinted(address indexed minter, uint256 indexed id, address indexed owner, uint256 bits);
    ...
    function mint(address owner, uint256 bits) public restricted {
        uint256 tokenID = nftMinted++;
        _bits[tokenID] = bits;
        _mint(owner, tokenID);

        emit NFTMinted(msg.sender, tokenID, owner, bits);
    }
